function fish_prompt --description 'Write out the prompt'
  ...
  set_color -b black
    printf '%s%s%s%s%s%s%s%s%s%s%s%s%s%s' (set_color normal) (set_color -o white) '⟨' (set_color green) $USER (set_color white) '|' (set_color -o cyan) (prompt_pwd) (set_color white) $git_info (set_color white) '⟩' (set_color normal)
